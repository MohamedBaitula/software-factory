...
find_validation_status() {
  ...
  local candidate
  local latest_run
  local run_project_dir

  slug="$(sf_config_slug "$project_name")"
  dir="$(logs_dir)"
  today="$(date +%F)"
  latest_run="$(sf_run_latest_id "$dir")"

  if [[ -n "$latest_run" ]]; then
    run_project_dir="$(sf_run_project_dir "$dir" "$latest_run" "$project_name")"
    for candidate in \
      "$run_project_dir/validation.status" \
      "$run_project_dir/validation.log" \
      "$run_project_dir/status.env"; do
      if [[ -f "$candidate" ]]; then
        printf '%s\n' "$candidate"
        return 0
      fi
...
validation_summary() {
  ...
  if status_file="$(find_validation_status "$project_name")"; then
    if [[ "$(basename "$status_file")" == "status.env" ]]; then
      first_line="$(awk -F= '$1=="status"{print $2; exit}' "$status_file" | tr -d '\r')"
    else
